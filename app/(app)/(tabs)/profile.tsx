...
export default function ProfileScreen() {
    ...
    function getNewAreaToday() {
        ...
        if (validPreviousAreas.length > 1) {
            ...
            const unifiedPreviousArea = turf.dissolve(
                turf.flatten(turf.featureCollection(validPreviousAreas as any))
            );
    ...
    useEffect(() => {
        ...
        // Add a small delay before executing heavy operations
        const timer = setTimeout(() => {
            const task = InteractionManager.runAfterInteractions(() => {
                if (isCancelled) return;

                try {
                    // Original calculation for smaller datasets
                    const unified = turf.dissolve(
                        turf.flatten(turf.featureCollection([...validAreas]))
                    );

                    if (unified) {
                        setTotalArea(turf.area(unified));
                    } else {
                        setTotalArea(turf.area(validAreas[0]));
                    }
                } catch (e) {
                    console.log("Area calculation error:", e);
                    // Fallback to simple calculation
                    try {
                        const singleArea = validAreas[0]
                            ? turf.area(validAreas[0])
                            : 0;
                        setTotalArea(singleArea);
                    } catch {
                        setTotalArea(0);
                    }
                }
